# Graph Report - Episodive  (2026-07-23)

## Corpus Check
- 561 files · ~1,116,828 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4281 nodes · 6295 edges · 430 communities (241 shown, 189 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 490 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f9c20a93`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Domain Model & Categories
- Clip Player DataSource Tests
- Flow Combine Utilities
- Search Feature (MVI)
- Podcast DAO & Follow Tests
- Onboarding Feature
- Design System Scrollbars
- Episode DAO Group Tests
- Architecture Documentation
- Enum Type Converters
- Home Feature (MVI)
- Download DI Module
- Library Screen Tests
- Episode LocalDataSource Tests
- Dual Player DI Module
- Episode Repository Impl
- Player Repository Tests
- Podcast Detail Feature
- User Preferences DataStore
- Episode Repository Interface
- Episode DAO
- Clip Feature (MVI)
- Player Repository Impl
- Episode Repository Tests
- Podcast DAO
- Community 25
- Community 26
- Community 27
- Community 28
- Community 29
- Community 30
- Community 31
- Community 32
- Community 33
- Community 34
- Community 35
- Community 36
- Community 37
- Community 38
- Community 39
- Community 40
- Community 41
- Community 42
- Community 43
- Community 44
- Community 45
- Community 46
- Community 47
- Community 48
- Community 49
- Community 50
- Community 51
- Community 52
- Community 53
- Community 54
- Community 55
- Community 56
- Community 57
- Community 58
- Community 59
- Community 60
- Community 61
- Community 62
- Community 63
- Community 64
- Community 65
- Community 66
- Community 67
- Community 68
- Community 69
- Community 70
- Community 71
- Community 72
- Community 73
- Community 74
- Community 75
- Community 76
- Community 77
- Community 78
- Community 79
- Community 80
- Community 81
- Community 82
- Community 83
- Community 84
- Community 85
- Community 86
- Community 87
- Community 88
- Community 89
- Community 90
- Community 91
- Community 92
- Community 93
- Community 94
- Community 95
- Community 96
- Community 97
- Community 98
- Community 99
- Community 100
- Community 101
- Community 102
- Community 103
- Community 104
- Community 105
- Community 106
- Community 107
- Community 108
- Community 109
- Community 110
- Community 111
- Community 112
- Community 113
- Community 114
- Community 115
- Community 116
- Community 117
- Community 118
- Community 119
- Community 120
- Community 121
- Community 122
- Community 123
- Community 124
- Community 125
- Community 126
- Community 127
- Community 128
- Community 129
- Community 130
- Community 131
- Community 132
- Community 133
- Community 134
- Community 135
- Community 136
- Community 137
- Community 138
- Community 139
- Community 140
- Community 141
- Community 142
- Community 143
- Community 144
- Community 145
- Community 146
- Community 147
- Community 148
- Community 149
- Community 150
- Community 151
- Community 152
- Community 153
- Community 154
- Community 155
- Community 156
- Community 157
- Community 158
- Community 159
- Community 160
- Community 161
- Community 162
- Community 163
- Community 164
- Community 165
- Community 166
- Community 167
- Community 168
- Community 169
- Community 170
- Community 171
- Community 172
- Community 173
- Community 174
- Community 175
- Community 176
- Community 177
- Community 178
- Community 179
- Community 180
- Community 181
- Community 182
- Community 183
- Community 184
- Community 185
- Community 186
- Community 187
- Community 188
- Community 189
- Community 190
- Community 191
- Community 192
- Community 193
- Community 194
- Community 195
- Community 196
- Community 197
- Community 198
- Community 199
- Community 200
- Community 201
- Community 202
- Community 203
- Community 204
- Community 205
- Community 206
- Community 207
- Community 208
- Community 209
- Community 210
- Community 211
- Community 212
- Community 213
- Community 214
- Community 215
- Community 216
- Community 217
- Community 218
- Community 219
- Community 220
- Community 221
- Community 222
- Community 223
- Community 224
- Community 225
- Community 226
- Community 227
- Community 228
- Community 229
- Community 230
- Community 231
- Community 232
- Community 233
- Community 234
- Community 235
- Community 236
- Community 237
- Community 238
- Community 239
- Community 240
- Community 241
- Community 242
- Community 243
- Community 244
- Community 245
- Community 246
- Community 247
- Community 248
- Community 249
- Community 250
- Community 251
- Community 252
- Community 253
- Community 254
- Community 255
- Community 256
- Community 257
- Community 258
- Community 259
- Community 260
- Community 261
- Community 262
- Community 263
- Community 264
- Community 265
- Community 266
- Community 267
- Community 268
- Community 269
- Community 270
- Community 271
- Community 272
- Community 273
- Community 274
- Community 275
- Community 276
- Community 277
- Community 278
- Community 279
- Community 280
- Community 281
- Community 282
- Community 283
- Community 284
- Community 285
- Community 286
- Community 287
- Community 288
- Community 289
- Community 290
- Community 291
- Community 292
- Community 293
- Community 294
- Community 295
- Community 296
- Community 297
- Community 298
- homeEntries
- Community 300
- Community 301
- WidgetActionCallback
- Community 303
- Community 304
- Community 305
- Community 306
- Community 307
- Community 308
- Community 309
- Community 310
- Community 311
- Community 312
- Community 313
- Community 314
- ToggleLikedEpisodeUseCase
- Community 316
- Community 317
- Community 318
- Community 319
- Community 320
- Community 321
- Community 322
- Community 323
- Community 324
- Community 325
- Community 326
- Community 327
- Community 328
- Community 329
- Community 332
- Community 333
- Community 334
- Community 335
- Community 336
- Community 337
- 구현 체크리스트 (기능 진행 상황)
- Community 339
- Community 340
- Community 342
- Community 344
- Community 345
- Community 346
- Community 347
- Community 348
- Community 349
- Community 403
- Community 404
- Community 405
- Community 407
- Community 408
- Community 413
- Community 414
- Community 415
- Community 416
- Community 417
- Community 418
- Community 419
- Community 420
- Community 421
- Community 422
- Community 423
- Community 424
- .getPagingSource
- Content
- graphify-grep-guard.sh
- RemoteUpdater 캐싱 패턴 (Copilot 가이드)

## God Nodes (most connected - your core abstractions)
1. `Episode` - 175 edges
2. `Category` - 161 edges
3. `EpisodeDaoTest` - 76 edges
4. `PlayerDataSourceImplTest` - 67 edges
5. `PodcastDaoTest` - 53 edges
6. `EpisodiveTheme()` - 53 edges
7. `EpisodeWithExtrasView` - 49 edges
8. `EpisodeDao` - 48 edges
9. `PlayerRepository` - 40 edges
10. `EpisodeLocalDataSourceTest` - 39 edges

## Surprising Connections (you probably didn't know these)
- `StateImage (Coil + Palette 대표색 추출)` --semantically_similar_to--> `백그라운드 동기화 (WorkManager + Hilt Worker)`  [INFERRED] [semantically similar]
  DESIGN.md → docs/portfolio/PORTFOLIO.md
- `EpisodiveNavHost()` --calls--> `clipEntries()`  [INFERRED]
  app/src/main/kotlin/io/jacob/episodive/navigation/EpisodiveNavHost.kt → feature/clip/src/main/kotlin/io/jacob/episodive/feature/clip/navigation/ClipNavigation.kt
- `EpisodiveNavHost()` --calls--> `homeEntries()`  [INFERRED]
  app/src/main/kotlin/io/jacob/episodive/navigation/EpisodiveNavHost.kt → feature/home/src/main/kotlin/io/jacob/episodive/feature/home/navigation/HomeNavigation.kt
- `EpisodiveNavHost()` --calls--> `searchEntries()`  [INFERRED]
  app/src/main/kotlin/io/jacob/episodive/navigation/EpisodiveNavHost.kt → feature/search/src/main/kotlin/io/jacob/episodive/feature/search/navigation/SearchNavigation.kt
- `EpisodiveApp()` --calls--> `EpisodiveBackground()`  [INFERRED]
  app/src/main/kotlin/io/jacob/episodive/ui/EpisodiveApp.kt → core/designsystem/src/main/kotlin/io/jacob/episodive/core/designsystem/component/Background.kt

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **팔레트 동적 테마링이 플레이어·미니플레이어·위젯 표면에 주입** — design_dynamic_theming, design_state_image, docs_portfolio_portfolio_palette_theme, design_mini_player, design_glance_widget [INFERRED 0.85]
- **CI 커버리지 파이프라인 (Kover→뱃지→리포트)** — github_workflows_android_ci, github_workflows_android_kover_report, github_workflows_android_coverage_badge, docs_coverage_report_summary, readme_coverage_badge [INFERRED 0.85]

## Communities (430 total, 189 thin omitted)

### Community 0 - "Domain Model & Categories"
Cohesion: 0.02
Nodes (114): model/Category.kt, Category, AFTER_SHOWS, ALTERNATIVE, ANIMALS, ANIMATION, ARTS, ASTRONOMY (+106 more)

### Community 2 - "Flow Combine Utilities"
Cohesion: 0.06
Nodes (19): combine(), Flow, FlowExtTest, FindInLibraryUseCase, Flow, LibraryFindResult, LibraryViewModelTest, R (+11 more)

### Community 3 - "Search Feature (MVI)"
Cohesion: 0.13
Nodes (13): Error, Flow, Podcast, StateFlow, ViewModel, Loading, NavigateToCategory, NavigateToEpisode (+5 more)

### Community 5 - "Onboarding Feature"
Cohesion: 0.06
Nodes (24): ChooseCategory, ChoosePodcast, Error, Flow, PagingData, Podcast, StateFlow, ViewModel (+16 more)

### Community 6 - "Design System Scrollbars"
Cohesion: 0.07
Nodes (42): DecorativeScrollbar(), DecorativeScrollbarThumb(), DraggableScrollbar(), DraggableScrollbarThumb(), Color, Modifier, Orientation, scrollbarThumbColor() (+34 more)

### Community 8 - "Architecture Documentation"
Cohesion: 0.08
Nodes (31): 브랜드 레드 #F5332C (dynamicColor=false), 팔레트 기반 다이내믹 컬러 테마링, FadingEdgeText + fadingEdgeMarquee, Glance 홈스크린 위젯 (색 스킴 브릿지), EpisodiveGradientBackground (사선 이중 그라데이션), 오버레이 미니플레이어 (PlayerBar), androidx.navigation3 (NavDisplay·멀티 백스택), Now in Android 파생 골격 (+23 more)

### Community 9 - "Enum Type Converters"
Cohesion: 0.05
Nodes (20): EpisodeTypeConverter, RecentSearchTypeConverter, EpisodeType, BONUS, FULL, TRAILER, Feed, Medium (+12 more)

### Community 10 - "Home Feature (MVI)"
Cohesion: 0.08
Nodes (19): ClickChannel, ClickPodcast, Error, HomeAction, HomeEffect, HomeState, HomeViewModel, StateFlow (+11 more)

### Community 11 - "Download DI Module"
Cohesion: 0.07
Nodes (13): DownloadModule, Context, CoroutineDispatcher, EpisodeDownloaderImpl, Flow, EpisodeDownloader, Flow, DownloadProgress (+5 more)

### Community 12 - "Library Screen Tests"
Cohesion: 0.10
Nodes (4): Flow, PagingData, Podcast, LibraryScreenTest

### Community 14 - "Dual Player DI Module"
Cohesion: 0.06
Nodes (4): RepositoryModule, Flow, Player, PlayerDataSource

### Community 15 - "Episode Repository Impl"
Cohesion: 0.11
Nodes (3): EpisodeRepositoryImpl, Flow, PagingData

### Community 17 - "Podcast Detail Feature"
Cohesion: 0.08
Nodes (17): Error, Factory, StateFlow, ViewModel, Loading, PlayEpisode, PodcastAction, PodcastEffect (+9 more)

### Community 18 - "User Preferences DataStore"
Cohesion: 0.06
Nodes (4): DataStore, Preferences, UserPreferencesStoreTest, TemporaryFolder

### Community 19 - "Episode Repository Interface"
Cohesion: 0.13
Nodes (5): EpisodeRepository, Flow, PagingData, model/Episode.kt, Episode

### Community 21 - "Clip Feature (MVI)"
Cohesion: 0.13
Nodes (13): ClickEpisode, ClickPodcast, ClipAction, ClipEffect, ClipPlayerState, ClipViewModel, StateFlow, ViewModel (+5 more)

### Community 22 - "Player Repository Impl"
Cohesion: 0.06
Nodes (4): Flow, Player, Repeat, PlayerRepositoryImpl

### Community 25 - "Community 25"
Cohesion: 0.06
Nodes (4): Flow, Player, Repeat, PlayerRepository

### Community 26 - "Community 26"
Cohesion: 0.09
Nodes (3): Flow, MediaItem, PlayerDataSourceImpl

### Community 28 - "Community 28"
Cohesion: 0.16
Nodes (18): Brush, ContentScale, adjustBrightness(), DominantRegion, Bottom, Center, Full, Left (+10 more)

### Community 29 - "Community 29"
Cohesion: 0.09
Nodes (3): EpisodeLocalDataSourceImpl, Flow, EpisodeEntity

### Community 31 - "Community 31"
Cohesion: 0.10
Nodes (5): PodcastApi, PodcastRemoteDataSource, PodcastRemoteDataSourceImpl, PodcastResponse, ResponseWrapper

### Community 32 - "Community 32"
Cohesion: 0.15
Nodes (3): SearchResult, Podcast, SearchScreenTest

### Community 33 - "Community 33"
Cohesion: 0.10
Nodes (15): ChannelAction, ChannelEffect, ChannelState, ChannelViewModel, ClickBack, ClickPodcast, Error, Factory (+7 more)

### Community 34 - "Community 34"
Cohesion: 0.16
Nodes (4): Flow, PagingSource, RoomDatabase, PodcastLocalDataSource

### Community 35 - "Community 35"
Cohesion: 0.11
Nodes (4): EpisodeLocalDataSource, Flow, PagingSource, RoomDatabase

### Community 39 - "Community 39"
Cohesion: 0.10
Nodes (11): Flow, Podcast, RecentSearchRepositoryImpl, Flow, Podcast, RecentSearchRepository, DeleteRecentSearchUseCase, EpisodeSearch (+3 more)

### Community 40 - "Community 40"
Cohesion: 0.12
Nodes (6): Flow, PagingSource, Flow, PagingSource, PodcastLocalDataSourceImpl, PodcastWithExtrasView

### Community 41 - "Community 41"
Cohesion: 0.12
Nodes (7): Flow, PagingSource, SoundbiteLocalDataSource, Flow, PagingSource, SoundbiteLocalDataSourceImpl, SoundbiteEntity

### Community 42 - "Community 42"
Cohesion: 0.10
Nodes (8): ChannelRemoteDataSource, ChannelRemoteDataSourceImpl, T, Context, CoroutineDispatcher, ChannelResponse, ChannelRemoteDataSourceTest, Context

### Community 43 - "Community 43"
Cohesion: 0.14
Nodes (3): EpisodeRemoteDataSource, EpisodeRemoteDataSourceImpl, EpisodeResponse

### Community 44 - "Community 44"
Cohesion: 0.16
Nodes (4): Factory, PodcastRemoteUpdater, PodcastRemoteUpdaterTest, PagingConfig

### Community 45 - "Community 45"
Cohesion: 0.08
Nodes (7): FeedDao, PagingSource, FeedLocalDataSource, PagingSource, FeedLocalDataSourceImpl, PagingSource, FeedEntity

### Community 46 - "Community 46"
Cohesion: 0.17
Nodes (3): UpdatePlayedEpisodeUseCase, Progress, ClipScreenTest

### Community 47 - "Community 47"
Cohesion: 0.27
Nodes (24): AllSectionContent(), CategorySection(), EpisodeRowSection(), FindBar(), FindOrFilter(), FollowedContent(), Flow, LazyListState (+16 more)

### Community 48 - "Community 48"
Cohesion: 0.13
Nodes (5): EpisodeRemoteUpdater, Factory, Flow, PagingSource, EpisodeRemoteUpdaterTest

### Community 50 - "Community 50"
Cohesion: 0.50
Nodes (7): ChannelFooter(), ChannelFooterPreview(), ChannelHeader(), ChannelRoute(), ChannelScreen(), Modifier, Podcast

### Community 51 - "Community 51"
Cohesion: 0.09
Nodes (23): CancelSleepTimer, ClickEpisode, ClickPodcast, CollapsePlayer, ExpandPlayer, Next, PlayerAction, PlayIndex (+15 more)

### Community 52 - "Community 52"
Cohesion: 0.15
Nodes (4): StateFlow, ViewModel, PlayerViewModel, Job

### Community 53 - "Community 53"
Cohesion: 0.23
Nodes (4): Flow, PagingData, Podcast, PodcastRepositoryImpl

### Community 55 - "Community 55"
Cohesion: 0.15
Nodes (4): Flow, PagingSource, PagingSource, EpisodeWithExtrasView

### Community 56 - "Community 56"
Cohesion: 0.23
Nodes (4): Flow, PagingData, Podcast, PodcastRepository

### Community 57 - "Community 57"
Cohesion: 0.15
Nodes (17): RecentNewFeed, RecentNewValueFeed, Podcast, toChapter(), toChapters(), toEpisode(), toEpisodeResponse(), toEpisodeResponses() (+9 more)

### Community 58 - "Community 58"
Cohesion: 0.14
Nodes (15): EpisodiveSeeker(), EpisodiveSeekerUncontrollablePreview(), Modifier, GetChaptersUseCase, model/Chapter.kt, Chapter, ui/Chapter.kt, ChapterItem() (+7 more)

### Community 59 - "Community 59"
Cohesion: 0.13
Nodes (13): AndroidApplicationConventionPlugin, Plugin, Project, AndroidLibraryConventionPlugin, Plugin, Project, configureKotlin(), configureKotlinAndroid() (+5 more)

### Community 60 - "Community 60"
Cohesion: 0.25
Nodes (18): EpisodiveDragHandle(), Modifier, CardSection(), ChapterSection(), ControlPanelBottom(), ControlPanelProgress(), EpisodeInfoSection(), Modifier (+10 more)

### Community 63 - "Community 63"
Cohesion: 0.32
Nodes (13): ui/Episode.kt, EpisodeClipItem(), EpisodeClipItemPreview(), EpisodeDetailItem(), EpisodeItem(), episodeItems(), EpisodesSection(), Modifier (+5 more)

### Community 64 - "Community 64"
Cohesion: 0.14
Nodes (7): Context, MonitorModule, ConnectivityManagerNetworkMonitor, Flow, Flow, NetworkMonitor, ConnectivityManagerNetworkMonitorTest

### Community 65 - "Community 65"
Cohesion: 0.13
Nodes (6): WidgetDataModule, Flow, WidgetDataReaderImpl, Flow, WidgetDataReader, WidgetDataReaderEntryPoint

### Community 66 - "Community 66"
Cohesion: 0.13
Nodes (6): SoundbiteApiTest, ChapterApi, SoundbiteApi, ApiModule, Retrofit, ChaptersResponse

### Community 67 - "Community 67"
Cohesion: 0.16
Nodes (3): DpSize, EpisodiveWidgetLayout, EpisodiveWidgetLayoutTest

### Community 68 - "Community 68"
Cohesion: 0.22
Nodes (7): Flow, PagingData, PagingSource, Response, RemoteUpdater, Entity, Output

### Community 70 - "Community 70"
Cohesion: 0.04
Nodes (44): 1. Enum 처리 (필수), 2. 데이터베이스 스키마 (Room v8), 3. API 인증 (Podcast Index), 4. 응답 래퍼, Clean Architecture 계층, CLI 도구 사용 가이드, Codebase Analysis Policy, Core 모듈 (11개) (+36 more)

### Community 71 - "Community 71"
Cohesion: 0.11
Nodes (16): GroupKey, CHANNEL, FEED_GUID, FEED_ID, FEED_URL, LIVE, MEDIUM, PERSON (+8 more)

### Community 72 - "Community 72"
Cohesion: 0.16
Nodes (8): androidx, EpisodeSyncWorker, EpisodeSyncWorkerTest, android, Context, TestWorkerFactory, CoroutineWorker, Result

### Community 73 - "Community 73"
Cohesion: 0.14
Nodes (12): DeepLinkEvent, FirstLaunch, Intent, StateFlow, ViewModel, Loading, MainActivityState, MainActivityViewModel (+4 more)

### Community 74 - "Community 74"
Cohesion: 0.06
Nodes (15): PodcastRepositoryTest, GetChannelByIdUseCase, Flow, GetChannelsUseCase, Flow, GetPodcastsByChannelUseCaseTest, model/Channel.kt, Channel (+7 more)

### Community 75 - "Community 75"
Cohesion: 0.13
Nodes (13): Error, Flow, PagingData, Podcast, StateFlow, ViewModel, LibraryEffect, LibraryState (+5 more)

### Community 76 - "Community 76"
Cohesion: 0.30
Nodes (14): ActionParameters, activityAutoplayIntent(), isMediaServiceRunning(), Context, Intent, PendingIntent, mainActivityComponent(), playPausePendingIntent() (+6 more)

### Community 77 - "Community 77"
Cohesion: 0.15
Nodes (5): ChannelRepositoryImpl, Flow, ChannelRepositoryTest, ChannelRepository, Flow

### Community 78 - "Community 78"
Cohesion: 0.13
Nodes (5): ChapterRemoteDataSource, ChapterRemoteDataSourceImpl, network/di/DataSourceModule.kt, DataSourceModule, ChapterRemoteDataSourceTest

### Community 81 - "Community 81"
Cohesion: 0.18
Nodes (17): Podcast, toEpisode(), toEpisodeEntities(), toEpisodeEntity(), toEpisodes(), toEpisodeWithExtrasView(), toEpisodeWithExtrasViews(), toFeedEntities() (+9 more)

### Community 83 - "Community 83"
Cohesion: 0.12
Nodes (5): Flow, Repeat, UserPreferencesDataSourceImpl, datastore/di/DataSourceModule.kt, DataSourceModule

### Community 84 - "Community 84"
Cohesion: 0.13
Nodes (8): theme/Background.kt, BackgroundTheme, DimensionTheme, EpisodiveTheme(), TintTheme, SelectableCategory, ChannelScreenTest, OnboardingScreenTest

### Community 86 - "Community 86"
Cohesion: 0.16
Nodes (11): EpisodiveNavHost(), Modifier, channelEntries(), ChannelRoute, NavKey, NavKey, libraryEntries(), LibraryRoute (+3 more)

### Community 87 - "Community 87"
Cohesion: 0.17
Nodes (5): EpisodeSyncNotificationHelperTest, Context, NewEpisodeResult, SyncNewEpisodesUseCase, NotificationManager

### Community 90 - "Community 90"
Cohesion: 0.15
Nodes (4): toUserData(), toUserPreferences(), UserPreferences, DataStoreMapperTest

### Community 91 - "Community 91"
Cohesion: 0.10
Nodes (8): DataStoreModule, Context, DataStore, Preferences, Flow, Repeat, UserPreferencesKeys, UserPreferencesStore

### Community 92 - "Community 92"
Cohesion: 0.21
Nodes (12): Playback, BUFFERING, ENDED, IDLE, READY, ClipRoute(), ClipScreen(), ClipScreenPreview() (+4 more)

### Community 93 - "Community 93"
Cohesion: 0.29
Nodes (7): LibrarySection, All, Followed, Liked, Preferred, RecentlyListened, Saved

### Community 94 - "Community 94"
Cohesion: 0.12
Nodes (6): FeedRemoteDataSource, FeedRemoteDataSourceImpl, toRecentFeedResponse(), toRecentFeedResponses(), RecentFeedResponse, FeedRemoteDataSourceTest

### Community 95 - "Community 95"
Cohesion: 0.17
Nodes (3): Flow, Repeat, UserRepositoryImpl

### Community 98 - "Community 98"
Cohesion: 0.23
Nodes (13): EpisodiveScaffold(), FadeTopBarLayout(), FadeTopBarLayoutContent(), FadeTopBarLayoutPreview(), ImageVector, LazyListState, Modifier, PaddingValues (+5 more)

### Community 99 - "Community 99"
Cohesion: 0.14
Nodes (13): FadingEdgeText(), Color, Modifier, TextStyle, FontFamily, FontStyle, FontWeight, TextAlign (+5 more)

### Community 100 - "Community 100"
Cohesion: 0.14
Nodes (3): Flow, Repeat, UserRepository

### Community 101 - "Community 101"
Cohesion: 0.23
Nodes (11): NowPlayingSnapshot, Bitmap, GlanceModifier, NowPlayingContent(), NowPlayingControlsRow(), NowPlayingHeader(), BrandBadge(), Bitmap (+3 more)

### Community 103 - "Community 103"
Cohesion: 0.16
Nodes (5): SoundbiteRemoteDataSource, SoundbiteRemoteDataSourceImpl, toSoundbiteResponse(), SoundbiteResponse, SoundbiteRemoteDataSourceTest

### Community 104 - "Community 104"
Cohesion: 0.23
Nodes (8): EpisodiveApp(), Modifier, SnackbarHostState, EpisodiveNavigationBar(), EpisodiveNavigationBarItem(), EpisodiveNavigationDefaults, Composable, Modifier

### Community 105 - "Community 105"
Cohesion: 0.18
Nodes (8): AndroidApplicationComposeConventionPlugin, Plugin, Project, AndroidLibraryComposeConventionPlugin, Plugin, Project, configureAndroidCompose(), CommonExtension

### Community 108 - "Community 108"
Cohesion: 0.20
Nodes (3): Flow, PagingSource, SoundbiteDao

### Community 109 - "Community 109"
Cohesion: 0.15
Nodes (3): Flow, Repeat, UserPreferencesDataSource

### Community 111 - "Community 111"
Cohesion: 0.22
Nodes (16): ClosedFloatingPointRange, calculateFadeRatio(), createTextPaint(), dialGesture(), DialState, drawCenterIndicator(), drawDialContent(), drawNumber() (+8 more)

### Community 112 - "Community 112"
Cohesion: 0.18
Nodes (8): EpisodiveApplication, ImageLoader, TimberTree, Application, Configuration, HiltWorkerFactory, StackTraceElement, Timber

### Community 113 - "Community 113"
Cohesion: 0.23
Nodes (7): Bundle, Intent, ListenableFuture, MainActivity, ComponentActivity, ComponentName, MediaController

### Community 114 - "Community 114"
Cohesion: 0.26
Nodes (5): Intent, PendingIntent, MediaNotificationService, ImmutableList, MediaSessionService

### Community 115 - "Community 115"
Cohesion: 0.20
Nodes (8): BottomBarDestination, CLIP, HOME, LIBRARY, SEARCH, EpisodiveAppState, CoroutineScope, rememberEpisodiveAppState()

### Community 116 - "Community 116"
Cohesion: 0.20
Nodes (7): AndroidApplicationJacocoConventionPlugin, Plugin, Project, AndroidLibraryJacocoConventionPlugin, Plugin, Project, configureKover()

### Community 117 - "Community 117"
Cohesion: 0.38
Nodes (11): ButtonColors, EpisodiveButton(), EpisodiveButtonContent(), EpisodiveButtonDefaults, EpisodiveOutlinedButton(), EpisodiveTextButton(), EpisodiveTextButtonPreview(), Composable (+3 more)

### Community 118 - "Community 118"
Cohesion: 0.20
Nodes (7): ImageModule, Context, ImageLoader, OkHttpClient, ImageCacheInterceptor, Interceptor, Response

### Community 119 - "Community 119"
Cohesion: 0.19
Nodes (17): ErrorScreen(), ErrorScreenPreview(), Modifier, GradientColors, CategorySelectionScreen(), CompletionScreen(), CompletionScreenPreview(), Flow (+9 more)

### Community 120 - "Community 120"
Cohesion: 0.19
Nodes (3): DaoModule, EpisodiveDatabase, RoomDatabase

### Community 121 - "Community 121"
Cohesion: 0.20
Nodes (8): DownloadStatusConverter, DownloadStatus, COMPLETED, DOWNLOADING, FAILED, PAUSED, PENDING, toDownloadStatus()

### Community 122 - "Community 122"
Cohesion: 0.23
Nodes (5): EpisodiveInterceptor, Interceptor, Response, sha1(), EpisodiveInterceptorTest

### Community 123 - "Community 123"
Cohesion: 0.25
Nodes (6): Bundle, ListenableFuture, MediaItem, MediaSessionCallback, MediaSession, SessionResult

### Community 124 - "Community 124"
Cohesion: 0.23
Nodes (5): DispatchersModule, CoroutineDispatcher, SystemTimeProvider, TimeProvider, Long

### Community 125 - "Community 125"
Cohesion: 0.18
Nodes (9): CacheableQuery, EpisodeQuery, FeedId, FeedUrl, Live, Person, PodcastGuid, Random (+1 more)

### Community 126 - "Community 126"
Cohesion: 0.18
Nodes (10): ByChannel, FeedGuid, FeedId, FeedUrl, Medium, PodcastQuery, Recent, RecentNew (+2 more)

### Community 129 - "Community 129"
Cohesion: 0.24
Nodes (9): Color, Modifier, WaveAnimationIcon(), WaveAnimationIconPreview(), ClipAnimationIconText(), ClipAnimationIconTextPreview(), EpisodiveIconText(), Color (+1 more)

### Community 130 - "Community 130"
Cohesion: 0.48
Nodes (6): HomeRoute(), HomeScreen(), HomeScreenPreview(), itemWithDivider(), Modifier, Podcast

### Community 131 - "Community 131"
Cohesion: 0.18
Nodes (10): Medium, AUDIOBOOK, BLOG, COURSE, FILM, MUSIC, NEWSLETTER, PODCAST (+2 more)

### Community 135 - "Community 135"
Cohesion: 0.40
Nodes (11): ui/Podcast.kt, Dp, Modifier, Podcast, PodcastDetailItem(), PodcastItem(), podcasts(), PodcastSimpleItem() (+3 more)

### Community 136 - "Community 136"
Cohesion: 0.18
Nodes (7): HidePlayerBottomSheet, Podcast, NavigateToPodcast, PlayerEffect, ShowPlayerBottomSheet, ShowUnsaveSnackbar, SleepTimerExpired

### Community 137 - "Community 137"
Cohesion: 0.26
Nodes (13): EpisodiveSearchBar(), EpisodiveSearchBarExpandPreview(), Modifier, LazyListState, Modifier, Podcast, RecentSearchesSection(), RecentSearchItem() (+5 more)

### Community 138 - "Community 138"
Cohesion: 0.20
Nodes (8): WidgetLoading(), ArtworkResult, EpisodiveWidget, Context, GlanceAppWidget, GlanceId, EpisodiveGlanceTheme(), SizeMode

### Community 139 - "Community 139"
Cohesion: 0.24
Nodes (5): LoadParams, LoadResult, PagingSource, PagingState, RecommendedPodcastPagingSource

### Community 140 - "Community 140"
Cohesion: 0.24
Nodes (5): LoadParams, LoadResult, PagingSource, PagingState, SoundbiteEpisodePagingSource

### Community 145 - "Community 145"
Cohesion: 0.25
Nodes (3): EpisodeSyncScheduler, EpisodeSyncSchedulerTest, Context

### Community 146 - "Community 146"
Cohesion: 0.25
Nodes (4): Context, WidgetDispatcher, GlanceWidgetDispatcher, WidgetDispatcherModule

### Community 149 - "Community 149"
Cohesion: 0.33
Nodes (8): EpisodiveIconButton(), EpisodiveIconButtonDefaults, EpisodiveIconProgressButton(), EpisodiveIconToggleButton(), IconButtonColors, Modifier, Shape, IconToggleButtonColors

### Community 151 - "Community 151"
Cohesion: 0.50
Nodes (7): PodcastSnapshot, FeedArea(), FeedCell(), FeedGrid(), FeedStrip(), Bitmap, GlanceModifier

### Community 153 - "Community 153"
Cohesion: 0.23
Nodes (13): component/Background.kt, EpisodiveBackground(), EpisodiveGradientBackground(), EpisodiveGradientBackgroundPreview(), Modifier, Flow, Modifier, PagingData (+5 more)

### Community 154 - "Community 154"
Cohesion: 0.15
Nodes (12): ClearQuery, ClickEpisode, ClickFind, ClickPlayingEpisode, ClickPodcast, LibraryAction, QueryChanged, SelectSection (+4 more)

### Community 155 - "Community 155"
Cohesion: 0.29
Nodes (7): CustomCommand, SEEK_BACKWARD, SEEK_FORWARD, TOGGLE_LIKE, fromAction(), CommandButton, SessionCommand

### Community 156 - "Community 156"
Cohesion: 0.29
Nodes (5): AndroidTestConventionPlugin, Plugin, Project, configureTestPackaging(), CommonExtension

### Community 160 - "Community 160"
Cohesion: 0.32
Nodes (6): Modifier, LoadingWheel(), LoadingWheelPreview(), Modifier, LoadingScreen(), LoadingScreenPreview()

### Community 162 - "Community 162"
Cohesion: 0.32
Nodes (4): OkHttpClient, Retrofit, NetworkModule, Gson

### Community 164 - "Community 164"
Cohesion: 0.46
Nodes (7): PagingSource, loadAsSnapshot(), loadNextPage(), loadPage(), loadPrevPage(), Key, Value

### Community 165 - "Community 165"
Cohesion: 0.36
Nodes (7): feedHeightOf(), FeedMode, GRID, NONE, STRIP, forSize(), nowPlayingThumbOf()

### Community 166 - "Community 166"
Cohesion: 0.43
Nodes (3): Bitmap, WidgetBackground, WidgetColorExtractor

### Community 167 - "Community 167"
Cohesion: 0.36
Nodes (7): calculate_percentage(), generate_markdown_report(), main(), parse_coverage(), Calculate coverage percentage, Generate a markdown report from coverage data, Parse a Jacoco XML file and extract coverage data

### Community 168 - "Community 168"
Cohesion: 0.52
Nodes (4): EpisodiveNavigationState, NavKey, rememberEpisodiveNavigationState(), NavEntry

### Community 172 - "Community 172"
Cohesion: 0.33
Nodes (3): database/di/DataSourceModule.kt, DataSourceModule, io

### Community 173 - "Community 173"
Cohesion: 0.14
Nodes (3): Flow, RecentSearchLocalDataSource, RecentSearchLocalDataSourceTest

### Community 174 - "Community 174"
Cohesion: 0.17
Nodes (11): ClearQuery, ClearRecentSearches, ClickCategory, ClickEpisode, ClickPodcast, ClickRecentSearch, ClickSearch, QueryChanged (+3 more)

### Community 175 - "Community 175"
Cohesion: 0.38
Nodes (6): EpisodiveViewToggleButton(), EpisodiveViewToggleContent(), EpisodiveViewToggleDefaults, Composable, Modifier, PaddingValues

### Community 177 - "Community 177"
Cohesion: 0.29
Nodes (5): ResponseWrapperDeserializer, JsonDeserializationContext, JsonDeserializer, JsonElement, Type

### Community 178 - "Community 178"
Cohesion: 0.57
Nodes (3): Context, ExoPlayer, PlayerModule

### Community 179 - "Community 179"
Cohesion: 0.33
Nodes (4): ExoPlayerRule, Description, ExoPlayer, TestWatcher

### Community 183 - "Community 183"
Cohesion: 0.47
Nodes (5): toSoundbite(), toSoundbites(), Soundbite, toSoundbite(), toSoundbites()

### Community 185 - "Community 185"
Cohesion: 0.40
Nodes (3): Description, TestWatcher, RoomDatabaseRule

### Community 186 - "Community 186"
Cohesion: 0.33
Nodes (4): GetFollowedPodcastsPagingUseCase, Flow, PagingData, Podcast

### Community 187 - "Community 187"
Cohesion: 0.33
Nodes (4): GetUserRecommendedPodcastsPagingUseCase, Flow, PagingData, Podcast

### Community 192 - "Community 192"
Cohesion: 0.40
Nodes (3): RecentFeed, toRecentFeed(), toRecentFeeds()

### Community 193 - "Community 193"
Cohesion: 0.33
Nodes (4): Repeat, ALL, OFF, ONE

### Community 194 - "Community 194"
Cohesion: 0.22
Nodes (5): All, NowPlayingChanged, WidgetUpdater, WidgetUpdateRequest, Context

### Community 195 - "Community 195"
Cohesion: 0.33
Nodes (4): Description, Retrofit, TestWatcher, RetrofitRule

### Community 196 - "Community 196"
Cohesion: 0.40
Nodes (3): toRecentNewValueFeedResponse(), toRecentNewValueFeedResponses(), RecentNewValueFeedResponse

### Community 197 - "Community 197"
Cohesion: 0.36
Nodes (8): EpisodiveCenterTopAppBar(), EpisodiveCenterTopAppBarPreview(), EpisodiveTopAppBar(), IconButtonColors, ImageVector, Modifier, TopAppBarScrollBehavior, TopAppBarColors

### Community 198 - "Community 198"
Cohesion: 0.40
Nodes (3): toTrendingFeedResponse(), toTrendingFeedResponses(), TrendingFeedResponse

### Community 199 - "Community 199"
Cohesion: 0.39
Nodes (7): AnnotatedString, addLinksToAnnotatedString(), filterOverlappingMatches(), findAllMatches(), HtmlTextContainer(), isValidMatch(), Color

### Community 200 - "Community 200"
Cohesion: 0.33
Nodes (4): DisabledOnWindowsRule, Description, Statement, TestRule

### Community 201 - "Community 201"
Cohesion: 0.40
Nodes (3): Description, TestWatcher, MainDispatcherRule

### Community 202 - "Community 202"
Cohesion: 0.33
Nodes (5): Error, LastPlaySnapshot, Loading, PlayerState, Success

### Community 203 - "Community 203"
Cohesion: 0.40
Nodes (5): fromName(), PlaybackControl, PLAY_PAUSE, SEEK_BWD, SEEK_FWD

### Community 205 - "Community 205"
Cohesion: 0.50
Nodes (3): AndroidFeatureConventionPlugin, Plugin, Project

### Community 206 - "Community 206"
Cohesion: 0.50
Nodes (3): AndroidRoomConventionPlugin, Plugin, Project

### Community 207 - "Community 207"
Cohesion: 0.50
Nodes (3): AndroidWidgetConventionPlugin, Plugin, Project

### Community 208 - "Community 208"
Cohesion: 0.50
Nodes (3): HiltConventionPlugin, Plugin, Project

### Community 209 - "Community 209"
Cohesion: 0.67
Nodes (3): MVI 패턴 (Copilot 가이드), Clean Architecture 계층 구조 (README), MVI 패턴 (README)

### Community 210 - "Community 210"
Cohesion: 0.40
Nodes (4): Dispatcher, EpisodiveDispatchers, Default, IO

### Community 211 - "Community 211"
Cohesion: 0.40
Nodes (4): EpisodivePlayers, Clip, Main, Player

### Community 214 - "Community 214"
Cohesion: 0.33
Nodes (3): toRecentNewFeedResponse(), toRecentNewFeedResponses(), RecentNewFeedResponse

### Community 215 - "Community 215"
Cohesion: 0.60
Nodes (4): EpisodiveSwipeDismissSnackbarHost(), EpisodiveSwipeDismissSnackbarPreview(), Modifier, SnackbarHostState

### Community 216 - "Community 216"
Cohesion: 0.50
Nodes (4): EpisodiveTab(), EpisodiveTabDefaults, EpisodiveTabRow(), Modifier

### Community 217 - "Community 217"
Cohesion: 0.40
Nodes (3): GetAllPlayedEpisodesPagingUseCase, Flow, PagingData

### Community 218 - "Community 218"
Cohesion: 0.40
Nodes (3): GetClipEpisodesPagingUseCase, Flow, PagingData

### Community 219 - "Community 219"
Cohesion: 0.40
Nodes (3): GetEpisodesByPodcastIdPagingUseCase, Flow, PagingData

### Community 220 - "Community 220"
Cohesion: 0.40
Nodes (3): GetLikedEpisodesPagingUseCase, Flow, PagingData

### Community 221 - "Community 221"
Cohesion: 0.40
Nodes (3): GetSavedEpisodesPagingUseCase, Flow, PagingData

### Community 223 - "Community 223"
Cohesion: 0.40
Nodes (3): GetFollowedPodcastsUseCase, Flow, Podcast

### Community 224 - "Community 224"
Cohesion: 0.40
Nodes (3): GetForeignTrendingPodcastsUseCase, Flow, Podcast

### Community 225 - "Community 225"
Cohesion: 0.40
Nodes (3): GetLocalTrendingPodcastsUseCase, Flow, Podcast

### Community 226 - "Community 226"
Cohesion: 0.40
Nodes (3): GetPodcastsByChannelUseCase, Flow, Podcast

### Community 227 - "Community 227"
Cohesion: 0.40
Nodes (3): GetPodcastUseCase, Flow, Podcast

### Community 228 - "Community 228"
Cohesion: 0.40
Nodes (3): GetRecentPodcastsUseCase, Flow, Podcast

### Community 229 - "Community 229"
Cohesion: 0.40
Nodes (3): GetTrendingPodcastsUseCase, Flow, Podcast

### Community 230 - "Community 230"
Cohesion: 0.40
Nodes (3): GetUserRecentPodcastsUseCase, Flow, Podcast

### Community 231 - "Community 231"
Cohesion: 0.40
Nodes (3): GetUserTrendingPodcastsUseCase, Flow, Podcast

### Community 233 - "Community 233"
Cohesion: 0.40
Nodes (3): Bitmap, Context, WidgetImageLoader

### Community 234 - "Community 234"
Cohesion: 0.60
Nodes (3): gradlew script, die(), warn()

### Community 239 - "Community 239"
Cohesion: 0.40
Nodes (3): TrendingFeed, toTrendingFeed(), toTrendingFeeds()

### Community 240 - "Community 240"
Cohesion: 0.50
Nodes (3): EpisodiveTagDefaults, EpisodiveTopicTag(), Modifier

### Community 241 - "Community 241"
Cohesion: 0.40
Nodes (4): EpisodiveChipDefaults, EpisodiveFilterChip(), Modifier, Shape

### Community 297 - "Community 297"
Cohesion: 0.67
Nodes (4): ui/Category.kt, CategoryItem(), CategoryItemPreview(), Modifier

### Community 298 - "Community 298"
Cohesion: 0.67
Nodes (3): clipEntries(), ClipRoute, NavKey

### Community 299 - "homeEntries"
Cohesion: 0.67
Nodes (3): homeEntries(), HomeRoute, NavKey

### Community 300 - "Community 300"
Cohesion: 0.67
Nodes (3): NavKey, searchEntries(), SearchRoute

### Community 301 - "Community 301"
Cohesion: 0.50
Nodes (3): EpisodiveWidgetReceiver, GlanceAppWidget, GlanceAppWidgetReceiver

## Knowledge Gaps
- **413 isolated node(s):** `기술 스택`, `git worktree 작업 시 필수: local.properties 복사`, `Core 모듈 (11개)`, `Feature 모듈 (8개)`, `컨벤션 플러그인 (build-logic/convention/)` (+408 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **189 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Episode` connect `Episode Repository Interface` to `Community 130`, `Search Feature (MVI)`, `Community 136`, `Community 137`, `Home Feature (MVI)`, `Download DI Module`, `Library Screen Tests`, `Dual Player DI Module`, `Episode Repository Impl`, `Podcast Detail Feature`, `Clip Feature (MVI)`, `Player Repository Impl`, `Community 25`, `Community 26`, `Community 27`, `Community 153`, `Community 30`, `Community 32`, `Community 39`, `Community 46`, `Community 47`, `Community 52`, `Community 313`, `Community 314`, `ToggleLikedEpisodeUseCase`, `Community 316`, `Community 57`, `Community 318`, `Community 63`, `Community 58`, `Community 60`, `Community 61`, `Community 62`, `Community 75`, `Community 81`, `Community 217`, `Community 218`, `Community 219`, `Community 220`, `Community 221`, `Community 222`, `Community 92`, `Community 249`, `Community 114`, `Community 242`, `Community 243`, `Community 244`, `Community 245`, `Community 247`, `Community 248`, `Community 246`, `Community 250`, `Community 254`?**
  _High betweenness centrality (0.343) - this node is a cross-community bridge._
- **Why does `Category` connect `Domain Model & Categories` to `Search Feature (MVI)`, `Onboarding Feature`, `Enum Type Converters`, `Library Screen Tests`, `Episode Repository Impl`, `Episode Repository Interface`, `Community 297`, `Community 47`, `Community 53`, `Community 56`, `Community 323`, `Community 75`, `Community 83`, `Community 91`, `Community 95`, `Community 100`, `Community 228`, `Community 229`, `Community 106`, `Community 236`, `Community 109`, `Community 255`?**
  _High betweenness centrality (0.125) - this node is a cross-community bridge._
- **Why does `EpisodiveTheme()` connect `Community 84` to `Community 129`, `Community 130`, `Community 135`, `Community 137`, `Library Screen Tests`, `Community 153`, `Community 27`, `Community 28`, `Community 30`, `Community 160`, `Community 32`, `Community 297`, `Community 46`, `Community 47`, `Community 50`, `Community 58`, `Community 60`, `Community 61`, `Community 62`, `Community 63`, `Community 197`, `Community 74`, `Community 215`, `Community 92`, `Community 98`, `Community 111`, `Community 113`, `Community 117`, `Community 119`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **What connects `기술 스택`, `git worktree 작업 시 필수: local.properties 복사`, `Core 모듈 (11개)` to the rest of the system?**
  _422 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Domain Model & Categories` be split into smaller, more focused modules?**
  _Cohesion score 0.017543859649122806 - nodes in this community are weakly interconnected._
- **Should `Clip Player DataSource Tests` be split into smaller, more focused modules?**
  _Cohesion score 0.030303030303030304 - nodes in this community are weakly interconnected._
- **Should `Flow Combine Utilities` be split into smaller, more focused modules?**
  _Cohesion score 0.06453634085213032 - nodes in this community are weakly interconnected._