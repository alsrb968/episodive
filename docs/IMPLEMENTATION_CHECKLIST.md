...
- ✅ 클립 공유 — 제목 + 클립 시작 지점 ([4.3](#43-공유--연동))
...
- ✅ 에피소드 목록 (Paging)
- ✅ 앨범 아트 기반 동적 테마
- ✅ 팟캐스트 공유 — 제목 + 웹사이트(없으면 RSS) 링크. 앱 전체가 `core/ui` 의
  `rememberShareLauncher()` 하나를 거친다
...
- ✅ 에피소드 공유 — 재생 컨트롤 아래 알약 바의 한가운데. 듣던 지점(10초 이상)과 링크를 함께 싣는다
...
- ⬜ 팔로우 팟캐스트 자동 저장
- ⬜ OPML 진입점 이관 — 지금은 보관함 상단 액션에 있다([4.3](#43-공유--연동)). 설정 화면이
  생기면 그리로 옮긴다. 그때도 이번에 만든 `secondaryActionIcon` 슬롯은 그대로 쓰인다
...
- ✅ 앱 딥링크 스킴 기반 공유 — 공유 문구에 웹 링크와 `episodive://` 를 나란히 싣는다
  ...
  에피소드는 원격 단건 조회로 채운다
- ✅ OPML import / export — 보관함 상단 액션(검색 아이콘 왼쪽)에서 시트를 열어 고른다.
  내보내기는 SAF 로 `episodive-yyyyMMdd.opml` 을 만들고(`core/data/.../opml/OpmlWriter.kt`),
  가져오기는 피드마다 **조회한 그 자리에서 팔로우까지 끝낸다**
  (`core/domain/.../usecase/opml/ImportOpmlUseCase.kt`). 진행률·부분 실패는 시트에 남고
  오프라인이면 남은 항목을 돌지 않고 끊는다
- ⬜ 에피소드 목록 행에서 공유 — 오버플로 메뉴·롱프레스 패턴이 저장소에 전례가 없어
  이번 범위에서 뺐다. 목록에서는 행을 눌러 플레이어를 연 뒤 공유한다
- ⬜ OPML 대량 가져오기 백그라운드화 — 지금은 `viewModelScope` 에서 돈다. 보관함은 하단 탭이라
  탭을 옮겨도 ViewModel 이 살아 있어 작업이 이어지지만, 앱을 벗어나면 끊긴다
...
| OPML 가져오기의 조회 | 피드를 **한 건씩** 조회한다(동시성 없음). `podcasts/batch/byguid` 는 network~RemoteDataSource 까지만 뚫려 있고 Repository 로 노출하려면 `PodcastQuery` 에 새 그룹키 설계가 얽혀 이번 범위에서 뺐다. 캐시 TTL 이 1시간이라 같은 파일을 다시 가져오면 네트워크는 타지 않는다 |
| OPML 내보내기의 MIME | `CreateDocument("application/octet-stream")` 로 연다. `text/xml` 을 주면 DocumentsUI 가 파일명에 `.xml` 을 덧붙여 `….opml.xml` 이 되기 때문이다. API 36 에뮬레이터에서 `episodive-20260826.opml` 이 그대로 저장되는 것을 확인했으나, 제공자마다 다를 수 있다 |
| OPML 의 HTML 엔티티 | 파서를 XML 표준 그대로 쓰므로 `&nbsp;` 같은 **HTML 전용 엔티티**가 제목에 든 파일은 읽기 실패로 끝난다(그런 파일은 엄밀히는 XML 이 아니다). 흔한 엔티티를 미리 정의해 받아 줄 수는 있으나, 어디까지 받아 줄지 기준이 없어 넣지 않았다 |
