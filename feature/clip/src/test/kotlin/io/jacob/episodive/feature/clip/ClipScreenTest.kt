...
class ClipScreenTest {
    ...
    fun whenTheIntermediateStatesAreSwallowed_theNextAdvanceStillFires() {
        ...
        // 없다 — 화면이 멈춰 있는 동안(예: 백그라운드로 나가 프레임 클록이 서 있는 사이)에
        // 오간 상태는 중간값이 남지 않아, 돌아왔을 때 화면이 보는 것은 ENDED → ENDED 다.
        //
    ...
    fun whenTheUserSwipesWhileAClipEnds_theClipTheyLandedOnIsNotSkipped() {
        ...
        // **여기에 "사용자가 정말 1 페이지에 착지했다" 는 전제 어써션을 두려다 네 번 실패했다.**
        // 요청열·요청 횟수·보이는 카드·좌표를 차례로 시도했는데 전부 공허했다. 이유는 하나다 —
        // 자동 넘김이 어차피 1 페이지로 옮겨 놓으므로, "사용자가 옮겨서 1" 과 "가만히 있었는데
        // 자동 넘김이 옮겨서 1" 의 **최종 상태가 완전히 같다.** 드래그 도중에 재려 해도 그때는
        // 배치가 갱신되지 않은 노드를 잡아 거꾸로 판정된다.
        //
        // 그래서 세우지 않고 적어 둔다: 이 테스트가 실제로 지키는 것은 **"건너뛰지 않는다"**
        // 하나다(소유권 가드를 지우면 아래에서 episodes[2] 가 잡혀 빨개진다 — 확인했다).
        // "사용자가 고른 자리에 있었다" 는 전제는 검증되지 않은 채로 둔다. 스와이프 동작이나
        // 페이지 피치가 바뀌어 그 전제가 무너지면 이 테스트는 조용히 힘을 잃는다.
        // 짝인 whenAClipEndsDuringADragThatSnapsBack 은 반대로 전제를 드래그 도중에 세울 수
        // 있어(그쪽은 자동 넘김이 아직 안 일어난 시점이다) assertCenteredClip 을 쓴다.
