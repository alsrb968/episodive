...
- **공유·딥링크**: 에피소드는 듣던 지점과, 클립은 시작 지점과 함께 공유. 문구에 `episodive://` 링크를
  함께 실어 받는 쪽이 앱의 그 지점으로 바로 들어온다
- **OPML 주고받기**: 구독 목록을 표준 OPML 파일로 내보내고, 다른 앱이 만든 파일을 들여와 팔로우에
  더한다. 새 권한 없이 SAF 로 파일을 고르고, 이미 팔로우 중인 것은 건드리지 않는다
