...
- 홈 미리보기와는 별도 캐시 그룹을 쓴다(`QueryScope.FULL`). 한 그룹을 공유하면 먼저 캐시를
  채운 쪽의 개수에 갇힌다.
- **스켈레톤은 첫 화면 분량만 기다린다.** 팟캐스트 섹션은 목록 API 가 피드 요약만 주기 때문에
  카드마다 상세 요청이 한 번 더 붙는데, 목록 전체를 미리 채우면 첫 화면이 51건의 요청을
  기다리게 된다. `FeedWindowPodcastPagingSource` 가 피드 목록만 먼저 받아 두고 **보이는
  페이지의 10건만** 채워, 첫 진입이 요청 왕복 두 번으로 끝난다. 나머지는 스크롤을 따라온다.
